...
class ScreenProgressTest extends TestCase
{
    use RefreshDatabase;
    use Funcs;

    public $routeName = "screen.progress";
    public $modelName = "screen_progress";
    public $resource = [
        "id",
        "name",
        "note",
    ...
    public function getParameters(bool $item = false, array $parameters = []): array
    {
        ...
        if ($item) $parameters[$this->modelName] = Screen_progress::factory()->create(["project_id" => $parameters["project"]->id]);
        return $parameters;
    }
